...
做这个东西的难点是：
1. 可参照项目不多，这个东西只有音视频网站会提供，然而他们都不会给这些功能开源(指的是这个项目刚开始干的时候，现在已经有很多类似的东西了)
2. 音视频本身就是个专业性很强的领域，各种复杂的算法，其实这个项目最难的就是一个既要又要的问题，既要提供尽可能多的语义上下文，又要保证切片在10秒内，所以核心难点就是如何做人声识别

# 与相同项目区别
1. 免费
2. 市面上多数免费的类似的都是基于Whisper的，感谢ai
3. 但ai太依赖算力，我们提供了Google云识别，准确性其实是很高的，而且不需要算力，有一个能上谷歌的网就行
4. 我们做的比pyTranscriber好（据我所知用Google云识别的也就还剩下pyTranscriber了）

# 基本设计

`视频` -> `FFmpeg预处理音频` -> `识别人声区域` -> `切片` -> `多线程上传Google Speech-To-Text API` -> `翻译` -> `生成SRT文件`
